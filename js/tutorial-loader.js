...
function getTooltipContent(termId) {
    const tooltipContent = {
        ...
        'byte': '8 bits grouped together (like 10110011)',
        'binary': 'A way of writing numbers using only 0s and 1s',
        'ASCII': 'A code that assigns each letter a number from 0-127',
        'bit-depth': 'Number of bits used to store each piece of data',
        'sampling-rate': 'How often measurements are taken when recording digital data',
        'encoding': 'Converting information into binary numbers',
        'decoding': 'Converting binary numbers back into readable information'
    };
...
const glossaryTerms = {
    ...
    'byte': 'A group of 8 bits. Can store a single character or a number from 0 to 255.',
    'binary': 'A number system that only uses 0s and 1s. It\'s how computers store all information.',
    'kilobyte': 'About 1000 bytes (actually 1024 bytes). Written as KB for short.',
    'megabyte': 'About 1 million bytes (1024 kilobytes). Written as MB for short.',
    'gigabyte': 'About 1 billion bytes (1024 megabytes). Written as GB for short.',
    
    // Binary Data Types
    'ASCII': 'A way to represent text using numbers. Each letter gets a unique number between 0 and 127.',
    'bit depth': 'How many bits are used to store each piece of data. More bits mean more detail can be stored.',
    'sampling rate': 'How many times per second a measurement is taken when converting real-world signals (like sound) to digital.',
    'binary code': 'Instructions or data written using only 1s and 0s that computers can understand.',
    
    // Common Terms
    'encoding': 'Converting information into a specific format, like turning letters into binary numbers.',
    'decoding': 'Converting encoded information back to its original form, like turning binary numbers back into letters.',
    'compression': 'Making data smaller by storing it more efficiently, like zipping a file.',
    'binary file': 'Any file stored as a sequence of binary numbers (which is actually all files on a computer).',
    'binary conversion': 'Changing numbers between binary and other number systems, like decimal (normal numbers).'
};
